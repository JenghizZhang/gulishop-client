<template>
    <div class="trade-container">
        ...
        <div class="trade">
            <div class="price">应付金额:　<span>¥10798.00</span></div>
            <div class="receiveInfo">
                寄送至:
                <span>{{
                    userAddressList.find((item) => item.isDefault).address
                }}</span>
                收货人：<span>{{
                    userAddressList.find((item) => item.isDefault).name
                }}</span>
                <span>{{
                    userAddressList.find((item) => item.isDefault).phone
                }}</span>
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        ...mapActions("user", ["getUserAddressList"]),
        changeDefault(index) {
            this.userAddressList.forEach((Addr) => (Addr.isDefault = false));
        ...
        async submitOrder() {
            ...
            if (result.code === 200) {
                // 2. 请求成功获取订单编号
                alert("创建订单成功，准备前往支付页面");
                // 3. 跳转到支付成功页面，需要带上订单编号
    ...
    data() {
        return {
            userAddressList: [
                {
                    name: "张三",
                    address: "北京市昌平区宏福科技园综合楼1层",
                    phone: 11000000000,
                    isDefault: false,
                },
                {
                    name: "李四",
                    address: "杭州市拱墅区宏福科技园综合楼2层",
                    phone: 11900000000,
                    isDefault: false,
                },
                {
                    name: "王五",
                    address: "深圳市宝安区宏福科技园综合楼3层",
                    phone: 12000000000,
                    isDefault: true,
                },
            ],
